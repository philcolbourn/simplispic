...
ATOM eval(ATOM exp, ATOM env){
  ...
  if ( is_load(exp)      ){  // (load "filename")
    ...
    */
    ...
    if ( f==NULL ) evalExit( "Could not open file",exp,env );
    FILE *old = in;
  ...
  if ( is_apply(exp) ){
    ...
    //SYM_PEEK( "Apply",proc );
    if ( is_null( proc ) )
      evalExit( "User function not defined in environment",exp,env );
    if ( is_atom( proc ) )
      evalExit( "User function returned an atom in environment",exp,env );
...
*/
  ...
  evalExit( "I can't do this for some reason: ",exp,env );
